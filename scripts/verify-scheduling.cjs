...
async function main() {
  ...
  const { JobOrchestrationService } = require('../dist/jobs/services/job-orchestration.service');
  ...
  const queuedPostMarket = [];
  const queue = {
    add: async (name, data, options) => {
      queuedPostMarket.push({ name, data, options });
      return {
        id: options.jobId,
        name,
  ...
  const orchestration = new JobOrchestrationService(config, queue, queue, queue, {
    latestCompletedSession: async () => '2026-09-18',
  });
  const requestedAt = new Date('2026-09-21T10:00:00.000Z');
  const immediate = await orchestration.enqueueLatestCompletedPipeline('MANUAL', requestedAt);
  assert.equal(immediate.jobId, `run-now-2026-09-18-${requestedAt.getTime()}`);
  assert.equal(queuedPostMarket[0].data.marketDate, '2026-09-18');
  assert.equal(queuedPostMarket[0].data.forceRun, true);
  ...
  try {
    ...
    assert.deepEqual(await monitorProcessor.process(fakeJob({ triggerSource: 'MANUAL' })), {
      monitored: 1,
    });
    ...
    global.Date = class extends realDate {
      constructor(...args) {
        super(...(args.length ? args : [beforeOpen]));
      }
    };
    assert.equal(
      (await monitorProcessor.process(fakeJob({ triggerSource: 'MANUAL' }))).status,
      'SKIPPED_OUTSIDE_MARKET_HOURS',
    );
    const saturday = new realDate('2026-09-19T04:30:00.000Z');
    global.Date = class extends realDate {
      constructor(...args) {
        super(...(args.length ? args : [saturday]));
      }
    };
    assert.equal(
      (await monitorProcessor.process(fakeJob({ triggerSource: 'MANUAL' }))).status,
  ...
  assert.equal(
    monitoringQueue.schedulers.size + postQueue.schedulers.size + eveningQueue.schedulers.size,
    0,
  );
  ...
  const analysis = new CandidateAnalysisService(
    ...
    { triageCandidate: async () => ({ success: true, routing: { escalate: false } }) },
    {
      reviewCandidate: async () => {
        throw new Error('DEEP must not run');
      },
    },
    {
  ...
  const pipelineConfig = {
    getOrThrow: (key) =>
      ({
        'marketData.universe': 'VERIFY',
        'scheduler.marketDataLookbackDays': 10,
        'providers.marketData': 'fixture',
      })[key],
  };
  const marketData = {
    syncInstruments: async () => {
      order.push('market-data-sync');
    },
    refresh: async (id) => {
  ...
  const scanner = {
    runDailyScan: async () => {
      scannerCalls += 1;
      order.push('scanner');
      return { run: { id: 'scan-1', marketDate: '2026-09-21' }, shortlist: [{ id: 'result-1' }] };
    },
  };
  const candidateEntity = {
    id: 'candidate-1',
    scanResultId: 'result-1',
    technicalSnapshot: { close: '100' },
    riskSnapshot: { accepted: true },
  };
  const candidateOrchestration = {
    createFromScanResult: async () => {
  ...
  const candidateQueue = {
    add: async (_name, data, opts) => {
      order.push('candidate-enqueue');
      queuedCandidates.push({ data, opts });
      return { id: opts.jobId };
    },
  };
  const summaryRuns = [];
  const pipelineJobSummary = {
    sendForRun: async (runId) => {
      summaryRuns.push(runId);
    },
  };
  const dailyPipeline = new DailyPipelineService(
    dataSource,
    runRepository,
    pipelineConfig,
  ...
  const firstRun = await dailyPipeline.run(
    { triggerSource: 'MANUAL', marketDate: '2026-09-21' },
  ...
  assert.deepEqual(summaryRuns, [firstRun.pipelineRunId]);
  ...
  await assert.rejects(() =>
    dailyPipeline.run({ triggerSource: 'MANUAL', marketDate: '2026-09-22' }),
  );
  assert.equal(
    scannerCalls,
    scannerCallsBeforeFailure,
    'freshness failure must abort before scanner',
  );
  assert.equal(rows.find((row) => row.marketDate === '2026-09-22').status, 'FAILED');
  const partial = await runRepository.save({
    id: 'partial-run',
    ...
    metadata: { activeJobId: 'verify-partial', activeRequestedAt: '2026-09-23T05:15:00.000Z' },
    startedAt: new Date('2026-09-23T05:15:00.000Z'),
    completedAt: null,
  });
  await dailyPipeline.recordCandidateSuccess(partial.id, {
    candidateId: 'candidate-ok',
    newsEnriched: true,
    fastAnalyzed: true,
    deepAnalyzed: false,
    qualified: true,
  ...
  assert.equal(
    (await runRepository.findOneBy({ id: partial.id })).status,
    'STARTED',
  ...
  assert.deepEqual(summaryRuns, [firstRun.pipelineRunId, partial.id]);
  ...
  for (const route of ['trade-monitor/run', 'post-market/run', 'run-now', 'evening/run']) {
    assert.ok(jobsController.includes(route));
  }
  console.log(
    'PASS: scheduler guards, stable registration, disablement, catch-up, candidate flow, and persistence constraints verified.',
  );
}
