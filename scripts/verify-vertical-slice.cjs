...
// Leaves clearly named VERIFY-* development records for inspection.
...
const sample = {
  symbol: 'RELIANCE', exchange: 'NSE', strategy: 'MOMENTUM_BREAKOUT', strategyVersion: 'v1',
  proposedEntry: '2920.00', proposedStop: '2850.00', target1: '3070.00', target2: '3180.00',
  suggestedQuantity: 20, quantScore: 87.4,
...
const create = (kind, extra = {}) => request('/candidates', 'POST', {
  ...sample, symbol: 'VERIFY-' + kind + '-' + suffix, ...extra,
}, 201);
...
async function unchanged(candidate) {
  ...
  assert.deepEqual(types(await request('/candidates/' + candidate.id + '/events')), ['CANDIDATE_CREATED']);
...
async function main() {
  ...
  assert.equal(initialRisk('99999999999999.9999', '0.0001', 2147483647),
  ...
  await request('/candidates/' + skipped.id + '/skip', 'POST', { reason: 'Setup is too extended' }, 201);
  ...
  await request('/candidates/' + skipped.id + '/buy', 'POST', { actualEntry: '2918.50', quantity: 20 }, 409);
  await request('/candidates/' + skipped.id + '/skip', 'POST', {}, 409);

  const accepted = await create('BUY');
  const trade = await request('/candidates/' + accepted.id + '/buy', 'POST',
    { actualEntry: '2918.50', quantity: 20 }, 201);
  const fetched = await request('/trades/' + trade.id);
  assert.equal((await request('/candidates/' + accepted.id)).status, 'ACCEPTED');
  for (const [key, value] of Object.entries({
    candidateId: accepted.id, symbol: accepted.symbol, strategy: sample.strategy,
    strategyVersion: 'v1', status: 'OPEN', plannedEntry: '2920.0000', actualEntry: '2918.5000',
    quantity: 20, initialStop: '2850.0000', currentStop: '2850.0000',
    target1: '3070.0000', target2: '3180.0000', initialRiskAmount: '1370.0000',
    currentPrice: null, realizedPnl: '0.0000', closedAt: null,
  ...
  assert.deepEqual(types(await request('/trades/' + trade.id + '/events')),
    ['CANDIDATE_CREATED', 'TRADE_OPENED']);
  await request('/candidates/' + accepted.id + '/buy', 'POST', { actualEntry: '2918.50', quantity: 20 }, 409);
  ...
  for (const entry of ['2850.00', '2849.99']) {
    await request('/candidates/' + invalid.id + '/buy', 'POST', { actualEntry: entry, quantity: 20 }, 400);
    await unchanged(invalid);
  }
  for (const body of [
    { actualEntry: 2918.50, quantity: 20 }, { actualEntry: '0', quantity: 20 },
    { actualEntry: '2918.50001', quantity: 20 }, { actualEntry: '2918.50', quantity: 0 },
    { actualEntry: '2918.50', quantity: 1.5 }, { actualEntry: '2918.50', quantity: '20' },
    { actualEntry: '2918.50', quantity: 2147483648 }, { actualEntry: '2918.50', quantity: 20, extra: true },
  ...
  const race = await Promise.all(Array.from({ length: 6 }, () => fetch(base + '/candidates/' + concurrent.id + '/buy', {
  ...
  const mixedResults = await Promise.all(['buy', 'skip'].map(action => fetch(base + '/candidates/' + mixed.id + '/' + action, {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
  ...
  assert.equal((await request('/trades?symbol=' + mixed.symbol)).length, mixedCandidate.status === 'ACCEPTED' ? 1 : 0);
  ...
  try {
    ...
    journal.record = async () => { throw new Error('Injected journal failure'); };
    try {
      await assert.rejects(service.buy(rollback.id, { actualEntry: '2918.50', quantity: 20 }),
        /Injected journal failure/);
      await unchanged(rollback);
      await assert.rejects(service.skip(rollback.id, 'Rollback verification'), /Injected journal failure/);
      await unchanged(rollback);
    ...
    } finally { journal.record = originalRecord; }

    // Verify DB uniqueness independently of service state checks.
    const repository = context.get(DataSource).getRepository(Trade);
    const duplicate = await repository.findOneByOrFail({ id: trade.id });
    await assert.rejects(repository.insert({ ...duplicate, id: randomUUID() }),
      error => error.driverError?.code === '23505');
  } finally { await context.close(); }

  console.log(JSON.stringify({
    result: 'PASS', skipCandidateId: skipped.id, buyCandidateId: accepted.id,
    tradeId: trade.id, initialRiskAmount: fetched.initialRiskAmount,
    verified: ['SKIP', 'BUY', 'filters', 'decimal precision', 'DTO validation', 'duplicate BUY',
      'concurrent BUY', 'BUY/SKIP race', 'invalid BUY unchanged', 'journal rollback', 'database uniqueness'],
    note: 'VERIFY-* records retained in local development database',
  }, null, 2));
}
