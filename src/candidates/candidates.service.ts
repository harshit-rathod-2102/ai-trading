...
export class CandidatesService {
  ...
  async skip(
    ...
    const candidate = await this.dataSource.transaction(async (manager) => {
      ...
      await this.journal.record(manager, {
        candidateId: id,
        eventType: TradeEventType.CANDIDATE_SKIPPED,
        source,
  ...
  async buy(
    ...
    const trade = await this.dataSource.transaction(async (manager) => {
      ...
      await this.journal.record(manager, {
        candidateId: id,
        tradeId: opened.id,
        eventType: TradeEventType.TRADE_OPENED,
        source,
        price: opened.actualEntry,
        quantity: opened.quantity,
        data: {
          previousStatus,
          status: CandidateStatus.ACCEPTED,
          plannedEntry: opened.plannedEntry,
          initialStop: opened.initialStop,
