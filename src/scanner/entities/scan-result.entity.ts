...
export class ScanResultRecord {
  ...
  @JoinColumn({ name: 'scan_run_id' }) scanRun!: ScanRun;
  ...
  @Column({ name: 'strategy_score', type: 'numeric', precision: 7, scale: 4 }) strategyScore!: string;
  @Column({ name: 'ranking_score', type: 'numeric', precision: 7, scale: 4 }) rankingScore!: string;
  @Column({ name: 'global_ranking_score', type: 'numeric', precision: 7, scale: 4 }) globalRankingScore!: string;
  ...
  @Column({ name: 'technical_snapshot', type: 'jsonb' }) technicalSnapshot!: TechnicalIndicatorSnapshot;
