...
export function calculatePortfolioConstraints(
  profile: TradingProfile, openTrades: readonly OpenTradeRiskView[], sector: string | null,
  ...
  for (const trade of openTrades) {
    const referencePrice = positive(trade.currentPrice ?? trade.actualEntry, `${trade.symbol} reference price`);
    const currentStop = positive(trade.currentStop, `${trade.symbol} current stop`);
    const positionValue = trade.currentPositionValue === null ? referencePrice.times(trade.quantity) :
    ...
    portfolioRiskBefore = portfolioRiskBefore.plus((downside.gt(0) ? downside : decimal(0, 'zero downside')).times(trade.quantity));
    if (sector !== null && trade.sector === sector) sectorExposure = sectorExposure.plus(positionValue);
  ...
  const availableCapital = rawAvailableCapital.gt(0) ? rawAvailableCapital : decimal(0, 'zero available capital');
  const rawRemainingRisk = maxPortfolioRisk.minus(portfolioRiskBefore);
  const remainingPortfolioRisk = rawRemainingRisk.gt(0) ? rawRemainingRisk : decimal(0, 'zero portfolio risk capacity');
  const maxSectorExposure = sector === null ? null : accountCapital.times(profile.maxSectorExposurePercent).div(100);
  const sectorExposureBefore = sector === null ? null : sectorExposure;
  const rawSectorCapacity = maxSectorExposure === null ? null : maxSectorExposure.minus(sectorExposure);
  const remainingSectorCapacity = rawSectorCapacity === null ? null :
    rawSectorCapacity.gt(0) ? rawSectorCapacity : decimal(0, 'zero sector capacity');
  const warningCodes: RiskWarningCode[] = [RiskWarningCode.CAPITAL_AVAILABILITY_APPROXIMATED];
  const warnings = ['Available capital is approximated as account capital minus open position value; cash balances and pending orders are unavailable'];
  if (missingCurrentPrice) {
    warningCodes.push(RiskWarningCode.PORTFOLIO_PRICE_DATA_STALE);
    warnings.push('One or more open trades have no current price; actual entry was used conservatively');
  }
  if (missingOpenTradeSector) {
    warningCodes.push(RiskWarningCode.OPEN_TRADE_SECTOR_UNAVAILABLE);
    warnings.push('One or more open trades have no sector metadata and cannot be assigned to sector exposure');
  }
  if (partiallyClosed) {
    warningCodes.push(RiskWarningCode.PARTIAL_QUANTITY_UNAVAILABLE);
    warnings.push('Stored quantity was used for partially closed trades because remaining quantity is not tracked yet');
  }
  return { capitalAllocated, availableCapital, portfolioRiskBefore, maxPortfolioRisk,
    remainingPortfolioRisk, maxPositionValue, sectorExposureBefore, maxSectorExposure,
    remainingSectorCapacity, warningCodes, warnings };
}
