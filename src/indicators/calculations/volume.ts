...
import { formatIndicator, mean, nonNegativeDecimal, periodIsValid, positiveDecimal } from './decimal';

function volumes(candles: readonly IndicatorCandle[]): (Decimal | null)[] {
  return candles.map((candle, index) => candle.volume === null ? null : nonNegativeDecimal(candle.volume, `candles[${index}].volume`));
}

export function calculateAverageVolume(candles: readonly IndicatorCandle[], period: number): string | null {
...
export function calculateVolumeRatio(candles: readonly IndicatorCandle[], period: number): string | null {
  periodIsValid(period);
  const parsed = volumes(candles);
...
export function calculateAverageTradedValue(candles: readonly IndicatorCandle[], period: number): string | null {
  periodIsValid(period);
  if (candles.length < period) return null;
  const values = candles.slice(-period).map((candle, index) => {
    if (candle.volume === null) return null;
    return positiveDecimal(candle.close, `window[${index}].close`)
      .times(nonNegativeDecimal(candle.volume, `window[${index}].volume`));
  });
