...
export function calculateBreadthSnapshot(
  ...
  return {
    totalUniverse: observations.length, totalEligible,
    totalExcluded: observations.length - totalEligible,
    aboveEma20, aboveEma50, aboveSma200,
...
export function calculateBreadthScore(snapshot: BreadthSnapshot): MarketRegimeComponent | null {
  const values = [snapshot.percentAboveEma20, snapshot.percentAboveEma50, snapshot.percentAboveSma200];
  ...
  if (percentages.some(value => value.lt(0) || value.gt(100))) throw new RangeError('breadth percentages must be between 0 and 100');
