...
export function calculateTrendScore(indicators: TechnicalIndicatorSnapshot): MarketRegimeComponent {
  ...
  const evidence = {
    closeAboveEma20: close.gt(ema20),
    closeAboveEma50: close.gt(ema50),
    closeAboveSma200: close.gt(sma200),
    ema20AboveEma50: ema20.gt(ema50),
    ema50AboveSma200: ema50.gt(sma200),
  ...
  const score = comparisons.reduce(
    (sum, [left, right]) => sum.plus(left.comparedTo(right) * 20),
    new RegimeDecimal(0),
  );
