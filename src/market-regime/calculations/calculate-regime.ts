...
} from '../models/market-regime-components.model';
...
export function calculateMarketRegime(
  ...
  config: MarketRegimeConfig = MARKET_REGIME_V1_CONFIG,
): MarketRegimeResult {
  validateEssentialInput(input);
  const warnings = new Set(input.warnings ?? []);
  const usableVix =
    input.vixFreshness === 'CURRENT' &&
    input.vixIndicators?.close !== null &&
    input.vixIndicators?.close !== undefined
      ? input.vixIndicators
      : undefined;
  if (!input.vixIndicators) warnings.add('INDIA_VIX_UNAVAILABLE');
  else if (input.vixFreshness !== 'CURRENT')
    warnings.add(`INDIA_VIX_NOT_CURRENT:${input.vixFreshness ?? 'UNKNOWN'}`);
  ...
  const sectorParticipation = input.sectorParticipation
    ? calculateSectorParticipationScore(input.sectorParticipation)
    : null;
  if (!breadth) warnings.add('BREADTH_UNAVAILABLE');
  else if (input.breadth!.totalExcluded)
    warnings.add(`BREADTH_EXCLUDED_INSTRUMENTS:${input.breadth!.totalExcluded}`);
  if (!sectorParticipation) warnings.add('SECTOR_PARTICIPATION_UNAVAILABLE');
    ...
    warnings.add(
      `SECTOR_PARTICIPATION_EXCLUDED_INSTRUMENTS:${input.sectorParticipation!.totalExcludedInstruments}`,
    );
  }
  ...
  return {
    ...
    components,
    reasons: componentReasons(components),
    warnings: [...warnings],
  };
}
...
function validateEssentialInput(input: MarketRegimeInput): void {
  if (
    !/^\d{4}-\d{2}-\d{2}$/.test(input.marketDate) ||
  ...
  if (missing.length)
    throw new MarketRegimeInputError(`NIFTY 50 indicators are incomplete: ${missing.join(', ')}`);
...
function weightedScore(components: MarketRegimeComponents, config: MarketRegimeConfig): Decimal {
  ...
  if (components.breadth)
    entries.push({ component: components.breadth, weight: config.weights.breadth });
  if (components.sectorParticipation) {
    entries.push({
      component: components.sectorParticipation,
      weight: config.weights.sectorParticipation,
    });
  }
  ...
  return entries
    .reduce(
      (sum, entry) =>
        sum.plus(decimal(entry.component.score, 'component score').times(entry.weight)),
      new RegimeDecimal(0),
    )
    .div(totalWeight);
}
...
function classify(
  ...
  const participation = [components.breadth, components.sectorParticipation]
    ...
    .some((component) =>
      decimal(component.score, 'participation score').lte(
        config.riskOffConfirmation.maximumParticipationScore,
      ),
    );
  const confirmedRiskOff =
    score.lte(config.classification.riskOff) &&
    decimal(components.trend.score, 'trend score').lte(
      config.riskOffConfirmation.maximumTrendScore,
    ) &&
    decimal(components.volatility.score, 'volatility score').lte(
      config.riskOffConfirmation.maximumVolatilityScore,
    ) &&
    participation;
...
function calculateConfidence(
  ...
  const coverage = new RegimeDecimal(65)
    .plus(hasVix ? 10 : 0)
    .plus(components.breadth ? 15 : 0)
    .plus(components.sectorParticipation ? 10 : 0);
  const values = [
    components.trend,
    components.momentum,
    components.volatility,
    components.breadth,
    components.sectorParticipation,
  ]
    .filter((component): component is MarketRegimeComponent => component !== undefined)
  ...
  const expectedDirection =
    regime === MarketRegime.BULLISH
      ? 1
      : regime === MarketRegime.BEARISH || regime === MarketRegime.RISK_OFF
        ? -1
        : 0;
  ...
  if (
    coverage.gte(config.confidence.highCoverage) &&
    agreement.gte(config.confidence.highAgreement)
  ) {
    return RegimeConfidence.HIGH;
  }
  if (
    coverage.gte(config.confidence.mediumCoverage) &&
    agreement.gte(config.confidence.mediumAgreement)
  ) {
...
function componentReasons(components: MarketRegimeComponents): string[] {
  return [
    ...
    ...(components.sectorParticipation
      ? [describe('sector participation', components.sectorParticipation.score)]
      : []),
  ];
}
...
function describe(label: string, value: string, risk = false): string {
  ...
  if (score.lte(-50))
    return `${label} ${risk ? 'are materially elevated' : 'is strongly negative'}`;
