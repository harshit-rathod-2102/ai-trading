...
export class MetaWhatsAppProvider implements MessagingProvider {
  ...
  async sendMessage(
    ...
    const fields = { module: MetaWhatsAppProvider.name, provider: 'meta-whatsapp',
      operation: 'sendMessage', endpoint: '/messages', messageType: message.messageType,
    ...
    try {
      ...
      this.logger.log({ event: 'provider.request.completed', ...fields,
      ...
      this.logger.error({ event: 'provider.request.failed', ...fields,
        ...
        ...structuredError(error) }, 'Meta WhatsApp send failed');
      throw error;
    }
  }
}
