...
export class GNewsProvider implements NewsProvider {
  ...
  async search(
      ...
      this.logger.debug(
        {
          ...
          module: GNewsProvider.name,
          provider: 'gnews',
          operation: 'search',
          query: summary(query.query),
          page: parameters.get('page'),
          resultCount: cached.articles.length,
        },
        'GNews cache hit',
      );
    ...
    try {
      ...
      if (!Array.isArray(response.articles))
        throw invalidResponse('GNews response is missing articles');
      ...
      if (malformed)
        this.logger.warn(
          { event: 'provider.response.partial', ...fields, rejectedCount: malformed },
          'Malformed GNews articles were skipped',
        );
      ...
      const articles = normalized
        .filter((article) => {
          if ((from && article.publishedAt < from) || (to && article.publishedAt > to))
            return false;
  ...
  private parameters(query: NewsQuery): URLSearchParams {
    ...
    if (!Number.isInteger(requestedLimit) || requestedLimit < 1)
      throw rejected('News limit must be a positive integer');
    const page = query.page ?? 1;
    if (!Number.isInteger(page) || page < 1 || page > 100)
      throw rejected('News page must be between 1 and 100');
    const sortBy = query.sortBy ?? NewsSortOrder.PUBLISHED_AT;
    if (!Object.values(NewsSortOrder).includes(sortBy))
      throw rejected('Unsupported news sort order');
...
function normalizeBoundary(value: string, endOfDay: boolean): string {
  ...
  if (!/(?:Z|[+-]\d{2}:\d{2})$/.test(value))
    throw rejected('News timestamp must include a timezone');
