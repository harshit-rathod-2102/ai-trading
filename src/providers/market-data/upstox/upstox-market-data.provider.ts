...
export class UpstoxMarketDataProvider implements MarketDataProvider {
  ...
  async getInstruments(
    ...
    this.logger.debug(
      ...
      'Upstox instrument request completed',
    );
    return filtered;
  }
  ...
  async getLatestCandle(
    ...
    const quote =
      Object.values(response.data).find(
        (value) => isRecord(value) && value.instrument_token === instrumentKey,
  ...
  private async loadInstrumentCatalog(
    ...
    if (!Array.isArray(payload))
      throw this.invalidResponse('Upstox instrument file is not an array');
  ...
  private async resolveInstrumentKey(
    ...
    if (!match) {
      throw new ProviderError(
        `Upstox instrument mapping was not found for ${reference.exchange}:${reference.symbol}`,
        {
          provider: this.id,
          code: ProviderErrorCode.REQUEST_REJECTED,
          retryable: false,
...
export function splitDailyRange(from: string, to: string): readonly { from: string; to: string }[] {
  ...
  if (start > end)
    throw new ProviderError('Historical range start must not exceed end', {
      provider: 'upstox',
      code: ProviderErrorCode.REQUEST_REJECTED,
      retryable: false,
    });
  ...
  let cursor = start;
  while (cursor <= end) {
    const exclusiveLimit = new Date(
      Date.UTC(cursor.getUTCFullYear() + 10, cursor.getUTCMonth(), cursor.getUTCDate()),
    );
