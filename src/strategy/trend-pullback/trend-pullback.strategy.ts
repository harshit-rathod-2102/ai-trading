...
import {
  ...
  calculateRollingLow,
} from '../../indicators/calculations/rolling-range';
...
export class TrendPullbackStrategy implements TradingStrategy {
  ...
  private evaluateSetup(e: Evaluation): StrategyResult {
    ...
    if (depth.lt(config.minimumDepthPercent))
      e.reject('PULLBACK_NOT_PRESENT', 'Retracement is smaller than the configured minimum');
    if (depth.gt(config.maximumDepthPercent))
      e.reject('PULLBACK_TOO_DEEP', 'Retracement exceeds the configured maximum');
    ...
    const baselineVolume = calculateAverageVolume(
      prior.slice(0, -config.contractionPeriod),
      config.priorVolumePeriod,
    )!;
    const contractionRatio = d(baselineVolume).isZero()
      ? null
      : d(recentVolume).div(baselineVolume);
    const recoveryVolume = d(previous.volume!).isZero()
      ? null
      : d(latest.volume!).div(previous.volume!);
    ...
    const components = {
      ...
      pullbackDepth: component(scoreBand(depth, config.depthBand), w.pullbackDepth, {
        recentHigh,
        close,
        retracementPercent: fixed(depth),
      }),
      supportZoneQuality: component(
        d(100).minus(scoreLinearRange(supportDistance, ...config.supportDistanceAtrRange)),
        w.supportZoneQuality,
        { supportLevel, ema20, ema50, distanceToSupportAtr: fixed(supportDistance) },
      ),
      relativeStrength: relativeStrength(e, w.relativeStrength),
      volumeContraction: component(
        contractionRatio === null
          ? d(0)
          : d(100).minus(scoreLinearRange(contractionRatio, ...config.contractionRatioRange)),
        w.volumeContraction,
        {
          recentAverageVolume: recentVolume,
          precedingAverageVolume: baselineVolume,
          contractionRatio: contractionRatio === null ? null : fixed(contractionRatio),
          recentPeriod: config.contractionPeriod,
          precedingPeriod: config.priorVolumePeriod,
        },
      ),
      renewedBuyingStrength: component(
        mean([
          scoreLinearRange(recovery, ...config.recoveryPercentRange),
          location,
          recoveryVolume === null
            ? d(0)
            : scoreLinearRange(recoveryVolume, ...config.recoveryVolumeRange),
        ]),
        w.renewedBuyingStrength,
        {
          close,
          previousClose: previous.close,
          recoveryPercent: fixed(recovery),
          closeLocationPercent: fixed(location),
          volumeVsPrevious: recoveryVolume === null ? null : fixed(recoveryVolume),
        },
      ),
    ...
    e.reasons.push(
      ...
      'Retracement lies within the configured pullback bounds',
    );
    if (recovery.gt(0)) e.reasons.push('Latest close improved over the preceding candle');
    if (contractionRatio?.lt(1))
      e.reasons.push('Recent pullback volume contracted versus its earlier baseline');
