...
export class MomentumBreakoutStrategy implements TradingStrategy {
  ...
  private evaluateSetup(e: Evaluation): StrategyResult {
    ...
    const setupContext = {
      ...
      recentBaseLow: priorLow,
      prior50DayHigh: longerHigh,
    };
    if (!d(close!).gt(ema50!) || !d(close!).gt(sma200!) || !d(ema50!).gt(sma200!)) {
      e.reject(
        'TREND_NOT_ESTABLISHED',
        'Breakout requires close above EMA50 and SMA200, and EMA50 above SMA200',
      );
    }
    if (!d(close!).gt(priorHigh))
      e.reject('NO_BREAKOUT', 'Latest close has not exceeded the prior 20-candle high');
    ...
    if (volumeRatio === null)
      throw new Error('Validated breakout volume baseline unexpectedly unavailable');
    ...
    const extension = mean([
      ...
      d(100).minus(
        scoreLinearRange(
          magnitude,
          config.breakoutMagnitudeBand[2],
          config.breakoutMagnitudeBand[3],
        ),
      ),
    ]);
    ...
    const components = {
      ...
      breakoutStructure: component(
        mean([
          scoreBand(magnitude, config.breakoutMagnitudeBand),
          location,
          d(100).minus(scoreLinearRange(baseWidth, ...config.baseWidthRange)),
        ]),
        w.breakoutStructure,
        {
          prior20DayHigh: priorHigh,
          prior50DayHigh: longerHigh,
          prior20DayLow: priorLow,
          close,
          breakoutPercent: fixed(magnitude),
          baseWidthPercent: fixed(baseWidth),
          closeLocationPercent: fixed(location),
          abovePrior50DayHigh: d(close!).gt(longerHigh),
        },
      ),
      volumeConfirmation: component(
        scoreLinearRange(d(volumeRatio), ...config.volumeRatioRange),
        w.volumeConfirmation,
        {
          currentVolume: latest.volume,
          priorVolumeRatio: volumeRatio,
          priorBaselinePeriod: config.volumePeriod,
        },
      ),
      ...
      extensionRisk: component(extension, w.extensionRisk, {
        distanceFromEma20Percent: fixed(ema20Distance),
        distanceFromEma50Percent: fixed(ema50Distance),
        distanceFromEma20Atr: fixed(atrDistance),
        breakoutPercent: fixed(magnitude),
      }),
    ...
    e.reasons.push(
      'Price closed above the prior 20-candle range high',
      'Long-term uptrend and prior liquidity filters passed',
    );
    if (d(volumeRatio).gt(1)) e.reasons.push('Volume expanded versus the prior 20-candle average');
    if (d(input.indicators.relativeStrength50!.excessReturnPercent).gt(0))
      e.reasons.push('Stock outperformed NIFTY over 50 aligned observations');
