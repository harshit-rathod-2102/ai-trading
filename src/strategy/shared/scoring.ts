...
export const clampScore = (value: Decimal): Decimal => value.lt(0) ? d(0) : value.gt(100) ? d(100) : value;
export const mean = (values: readonly Decimal[]): Decimal => values.reduce((sum, value) => sum.plus(value), d(0)).div(values.length);
export const distancePercent = (value: Decimal.Value, baseline: Decimal.Value): Decimal => d(value).div(baseline).minus(1).times(100);
...
export function scoreBand(value: Decimal, [minimum, preferredLow, preferredHigh, maximum]: ScoreBand): Decimal {
...
export function component(score: Decimal, weight: string, evidence: StrategyEvidence): StrategyScoreComponent {
  const bounded = clampScore(score);
  return { score: fixed(bounded), weight: fixed(weight),
    weightedContribution: fixed(bounded.times(weight).div(100)), evidence };
}

export function weightedScore(components: Readonly<Record<string, StrategyScoreComponent>>): Decimal {
