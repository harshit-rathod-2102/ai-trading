...
export function trendQuality(input: StrategyInput, weight: string): StrategyScoreComponent {
  ...
  const score = mean(
    pairs.map(([left, right]) => (d(left).gt(right) ? d(100) : d(left).eq(right) ? d(50) : d(0))),
  );
  return component(score, weight, {
    close,
    ema20,
    ema50,
    sma200,
...
export function relativeStrength(e: Evaluation, weight: string): StrategyScoreComponent {
  const {
    ...
    relativeStrength50: rs50,
    relativeStrength126: rs126,
  } = e.input.indicators;
  const values = [rs20!, rs50!, ...(rs126 ? [rs126] : [])];
  return component(
    mean(
      values.map((rs) =>
        scoreLinearRange(d(rs.excessReturnPercent), ...e.config.relativeStrengthRange),
      ),
    ),
    weight,
    {
      excessReturn20Percent: rs20!.excessReturnPercent,
      excessReturn50Percent: rs50!.excessReturnPercent,
...
export function momentum(e: Evaluation, weight: string): StrategyScoreComponent {
  ...
  return component(
    mean([
      scoreBand(d(rsi14!), e.config.rsiBand),
      scoreLinearRange(d(roc20!), ...e.config.roc20Range),
      scoreLinearRange(d(roc50!), ...e.config.roc50Range),
    ]),
    weight,
    { rsi14, roc20, roc50 },
  );
...
export function volatility(e: Evaluation, weight: string): StrategyScoreComponent {
  ...
  return component(scoreBand(d(normalizedAtr14!), e.config.normalizedAtrBand), weight, {
    normalizedAtr14,
    atr14,
  });
}
...
export function regimeFit(e: Evaluation, weight: string): StrategyScoreComponent {
  return component(d(e.config.regimeScores[e.input.marketRegime.regime]), weight, {
    regime: e.input.marketRegime.regime,
    version: e.input.marketRegime.version,
    confidence: e.input.marketRegime.confidence,
  });
}

export function sectorStrength(e: Evaluation, weight: string): StrategyScoreComponent {
  return component(d(e.sectorScore), weight, {
    sector: e.input.instrument.sector ?? null,
    suppliedStrength: e.input.sectorContext?.strengthScore ?? null,
    usable:
      !e.warningCodes.includes('SECTOR_CONTEXT_INVALID') &&
      !e.warningCodes.includes('SECTOR_CONTEXT_UNAVAILABLE'),
  });
}
