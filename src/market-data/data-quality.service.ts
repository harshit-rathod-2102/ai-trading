  ...
  assess(
    ...
    for (const row of rows) {
      ...
      if (row.sessionDate >= calendar.coverageFrom && row.sessionDate <= calendar.coverageTo) {
        ...
        else if (Date.parse(session.closeAt) > now.getTime())
          issues.push('Session is not yet closed');
      }
    ...
    const freshness = !latestExpectedSession
      ? 'UNKNOWN'
      : latest?.sessionDate === latestExpectedSession
        ? 'CURRENT'
        : 'STALE';
    const zeroVolumeSessions =
      instrument.type === InstrumentType.EQUITY
        ? rows.filter((row) => row.volume === '0').map((row) => row.sessionDate)
        : [];
    const synthetic =
      rows.some((row) => row.isSynthetic) || Boolean(latest?.isSynthetic) || calendar.isSynthetic;
    const unknownSessionDates = rows
      .filter(
        (row) => row.sessionDate < calendar.coverageFrom || row.sessionDate > calendar.coverageTo,
      )
    ...
    if (adjustmentBases.includes('UNADJUSTED'))
      warnings.push('CORPORATE_ACTION_ADJUSTMENT_NOT_VERIFIED');
