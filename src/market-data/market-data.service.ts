...
export class MarketDataService {
  ...
  async latestPrice(instrumentId: string): Promise<LatestMarketPrice> {
    ...
    const quote = await this.providerCall(
      'getLatestPrice',
      (signal) =>
  ...
  async syncInstruments() {
    ...
    this.logger.log(
      {
        event: 'market_data.sync.started',
        module: MarketDataService.name,
        operation: 'syncInstruments',
        provider: this.provider.id,
      },
      'Instrument sync started',
    );
    try {
      const catalog = await this.providerCall('getInstruments', (signal) =>
        this.provider.getInstruments(
          {
      ...
      this.logger.log(
        {
          event: 'market_data.sync.completed',
          module: MarketDataService.name,
          operation: 'syncInstruments',
          provider: this.provider.id,
          receivedCount: catalog.length,
    ...
    try {
      ...
      const result = await this.dataSource.transaction(async (manager) => {
        ...
        if (!instrument.isActive)
          throw new ConflictException('Inactive instruments cannot be refreshed');
        symbol = instrument.symbol;

        const calendar = await this.calendar(instrument.exchange, from, to);
        if (from < calendar.coverageFrom || to > calendar.coverageTo) {
          throw new BadRequestException(
            'Provider calendar does not cover the complete requested range',
          );
        }
        const bars = await this.providerCall(
          ...
          { symbol: instrument.symbol, instrumentId, from, to, interval: CandleInterval.ONE_DAY },
        );
        if (!Array.isArray(bars))
          throw new BadGatewayException('Provider did not return daily candles');
        const now = new Date();
        ...
        for (const bar of bars) {
          ...
          if (!bar || !expectedSet.has(bar.sessionDate))
            errors.push('Date is outside requested completed sessions');
          if (bar && seen.has(bar.sessionDate))
            errors.push('Provider returned a duplicate session');
        ...
        if (issues.length || missing.length) {
          this.logger.warn(
            {
              event: 'market_data.quality.failed',
              module: MarketDataService.name,
              operation: 'refreshDailyCandles',
              provider: this.provider.id,
              symbol: instrument.symbol,
        ...
        const incompatible = await repository
          ...
          .andWhere('(candle.isSynthetic <> :synthetic OR candle.adjustmentBasis <> :basis)', {
            synthetic: this.provider.isSynthetic,
            basis: this.provider.adjustmentBasis,
          })
          .getExists();
        if (incompatible)
          throw new ConflictException(
            'Cannot mix synthetic/live data or adjustment bases; deliberate history replacement is required',
          );
        ...
        if (bars.length) {
          const values = bars.map((bar) =>
            repository.create({
              ...
              open: new Decimal(bar.open).toFixed(4),
              high: new Decimal(bar.high).toFixed(4),
              low: new Decimal(bar.low).toFixed(4),
              close: new Decimal(bar.close).toFixed(4),
              volume: bar.volume,
              provider: this.provider.id,
              isSynthetic: this.provider.isSynthetic,
              adjustmentBasis: this.provider.adjustmentBasis,
      ...
      this.logger.log(
        {
          ...
          from,
          to,
          receivedCount: result.upserted,
          writtenCount: result.upserted,
          rejectedCount: 0,
          durationMs: elapsedMilliseconds(startedAt),
          status: 'completed',
        },
        'Candle sync completed',
      );
      return result;
    } catch (error: unknown) {
      this.logger.error(
  ...
  private async calendar(exchange: string, from?: string, to?: string): Promise<TradingCalendar> {
    const calendar = await this.providerCall(
      'getTradingCalendar',
      (signal) =>
        this.provider.getTradingCalendar(
          {
    ...
    if (issues.length)
      throw new BadGatewayException({ message: 'Invalid provider calendar', issues });
    return calendar;
  }
