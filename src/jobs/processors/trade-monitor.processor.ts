...
export class TradeMonitorProcessor extends WorkerHost {
  ...
  constructor(private readonly config: ConfigService, private readonly tradingDays: TradingDayService,
    private readonly monitor: TradeMonitorService) { super(); }
  ...
  async process(job: Job<MarketJobData>) {
    ...
    const fields = { jobId: job.id, jobName: job.name, marketDate: clock.marketDate,
      attempt: job.attemptsMade + 1, triggerSource: job.data.triggerSource };
    this.logger.log({ event: 'job.started', ...fields }, 'Trade-monitor job started');
    try {
      const inHours = isWithinTimeRange(clock.hhmm,
      ...
      if (!inHours || this.tradingDays.isWeekend(clock.marketDate)) {
        this.logger.log({ event: 'job.skipped', ...fields,
          reason: 'SKIPPED_OUTSIDE_MARKET_HOURS' }, 'Trade-monitor job skipped');
      ...
      if (!await this.tradingDays.isTradingDay(clock.marketDate)) {
        this.logger.log({ event: 'job.skipped', ...fields,
          reason: 'SKIPPED_NON_TRADING_DAY' }, 'Trade-monitor job skipped');
      ...
      this.logger.log({ event: 'job.completed', ...fields,
        durationMs: elapsedMilliseconds(startedAt) }, 'Trade-monitor job completed');
      return result;
    } catch (error: unknown) {
      this.logger.error({ event: 'job.failed', ...fields, durationMs: elapsedMilliseconds(startedAt),
        ...structuredError(error) }, 'Trade-monitor job failed');
      throw error;
    }
  }
}
