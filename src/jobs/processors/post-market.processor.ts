...
export class PostMarketProcessor extends WorkerHost {
  ...
  constructor(private readonly pipeline: DailyPipelineService, private readonly config: ConfigService) { super(); }

  async process(job: Job<PostMarketJobData>) {
    const startedAt = performance.now();
    const marketDate = job.data.marketDate ?? marketClock(new Date(),
      this.config.getOrThrow<string>('scheduler.timezone')).marketDate;
    const data = { ...job.data, marketDate };
    const fields = { jobId: job.id, jobName: job.name, marketDate,
      attempt: job.attemptsMade + 1, triggerSource: data.triggerSource };
    ...
    try {
      ...
      this.logger.log({ event: 'job.completed', ...fields,
        durationMs: elapsedMilliseconds(startedAt) }, 'Post-market job completed');
      return result;
    } catch (error: unknown) {
      this.logger.error({ event: 'job.failed', ...fields, durationMs: elapsedMilliseconds(startedAt),
        ...structuredError(error) }, 'Post-market job failed');
      throw error;
    }
  }
}
