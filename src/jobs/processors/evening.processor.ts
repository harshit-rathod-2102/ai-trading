...
export class EveningProcessor extends WorkerHost {
  ...
  constructor(private readonly evening: EveningService, private readonly config: ConfigService) { super(); }
  async process(job: Job<MarketJobData>) {
    const startedAt = performance.now();
    const marketDate = marketClock(new Date(),
    ...
    try {
      ...
      this.logger.log({ event: 'job.completed', ...fields,
        durationMs: elapsedMilliseconds(startedAt) }, 'Evening job completed');
      return result;
    } catch (error: unknown) {
      this.logger.error({ event: 'job.failed', ...fields, durationMs: elapsedMilliseconds(startedAt),
        ...structuredError(error) }, 'Evening job failed');
      throw error;
    }
  }
}
