...
export class NewsEnrichmentService {
  ...
  async enrichCandidate(candidateId: string): Promise<NewsEnrichmentResult> {
    ...
    if (running) {
      this.logger.log({ event: 'news.enrichment.reused', module: NewsEnrichmentService.name,
        operation: 'enrichCandidate', candidateId, provider: this.providerName,
        reason: 'in_flight' }, 'Joined in-flight candidate news enrichment');
      return running;
    }
  ...
  private async execute(candidateId: string): Promise<NewsEnrichmentResult> {
    ...
    this.logger.log({ event: 'news.enrichment.started', module: NewsEnrichmentService.name,
      operation: 'enrichCandidate', candidateId, provider: this.providerName },
    ...
    if (existing) {
      this.logger.log({ event: 'news.enrichment.reused', ...this.logContext(context),
        queryCount: existing.queries.length, articleCount: existing.articleCount,
        durationMs: elapsedMilliseconds(startedAt), reason: 'fresh_snapshot' },
      'Fresh candidate news snapshot reused');
      return this.success(candidateId, existing, true);
    }

    if (!this.provider) {
      return this.providerFailure(context, startedAt,
        NewsEnrichmentErrorCode.NEWS_PROVIDER_UNAVAILABLE, true,
    ...
    try {
      ...
      return this.providerFailure(context, startedAt, mapped.code, mapped.retryable, mapped.warning, error);
    }
    ...
    try {
      ...
      return this.providerFailure(context, startedAt,
        NewsEnrichmentErrorCode.NEWS_NORMALIZATION_FAILED, true,
        'Normalized news evidence was invalid; no snapshot was stored', error);
    }
    ...
    try {
      ...
      const event = result.articleCount === 0 ? 'news.enrichment.no_articles' : 'news.enrichment.completed';
      this.logger.log({ event, ...this.logContext(context), queryCount: 1,
        providerResultCount: processed.providerResultCount,
        deduplicatedCount: processed.duplicateCount,
        filteredCount: processed.staleCount + processed.relevanceFilteredCount,
        finalArticleCount: result.articleCount, articleCount: result.articleCount,
        reusedExistingSnapshot: result.reusedExistingSnapshot,
        durationMs: elapsedMilliseconds(startedAt), status: 'completed' },
      result.articleCount === 0 ? 'Candidate news enrichment completed without articles' :
  ...
  private processArticles(
    ...
    const recent = input.filter(article => {
      ...
      if (Number.isNaN(published.getTime())) throw new TypeError('Article publication timestamp is invalid');
      const allowed = published >= from && published <= to;
      if (!allowed) staleCount += 1;
      return allowed;
    });
    if (staleCount) warnings.push(`${staleCount} article(s) outside the configured lookback were excluded.`);
    ...
    for (const article of recent) {
      ...
      if ((providerId && seenProviderIds.has(providerId)) || seenUrls.has(url) || seenFallbacks.has(fallback)) {
        duplicateCount += 1;
        continue;
      }
    ...
    if (matched.length > 0) {
      ...
      if (relevanceFilteredCount) {
        warnings.push(`${relevanceFilteredCount} article(s) without a company or symbol match were excluded.`);
      }
    } else if (unique.length > 0) {
      warnings.push('Article text had no exact company or symbol match; results were retained as uncertain evidence.');
    }
  ...
  private freshSnapshot(candidate: TradeCandidate, now: Date): CandidateNewsSnapshot | null {
    if (!candidate.newsEnrichedAt || !isRecord(candidate.newsSnapshot) ||
        candidate.newsSnapshot.version !== config.version) return null;
  ...
  private providerFailure(
    ...
    this.logger.error({ event: 'news.enrichment.failed', ...this.logContext(context),
      queryCount: 1, articleCount: 0, errorCode, retryable,
      durationMs: elapsedMilliseconds(startedAt), status: 'failed',
      ...(error ? structuredError(error) : {}) }, 'Candidate news enrichment failed');
    return {
  ...
  private logFailure(
    ...
    this.logger.error({ event: 'news.enrichment.failed', module: NewsEnrichmentService.name,
      operation: 'enrichCandidate', candidateId, provider: this.providerName,
...
function providerFailure(error: unknown): {
  ...
  switch (error.code) {
    ...
    case ProviderErrorCode.TIMEOUT:
      return { code: NewsEnrichmentErrorCode.NEWS_PROVIDER_TIMEOUT, retryable: true,
        warning: 'The news-provider request timed out; enrichment remains retryable.' };
    case ProviderErrorCode.INVALID_RESPONSE:
      return { code: NewsEnrichmentErrorCode.NEWS_NORMALIZATION_FAILED, retryable: true,
    ...
    case ProviderErrorCode.REQUEST_REJECTED:
      return { code: NewsEnrichmentErrorCode.NEWS_PROVIDER_REJECTED, retryable: false,
...
function relevanceAliases(context: CandidateContext): readonly string[] {
  ...
  if (company) {
    ...
    const withoutSuffix = normalized.replace(/\b(?:limited|ltd|plc|incorporated|inc|corporation|corp)\b/g, '')
      .replace(/\s+/g, ' ').trim();
...
function articleMatches(article: CandidateNewsArticleSnapshot, aliases: readonly string[]): boolean {
...
function canonicalUrl(value: string): string {
  ...
  if (!['http:', 'https:'].includes(url.protocol)) throw new TypeError('Article URL must use HTTP or HTTPS');
  url.hash = '';
